...
export default function EditItem({ item }: { item: any }) {
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-white flex items-center gap-2">
                                <Package size={20} className="text-indigo-400" /> Edit Item
                            </h1>
                            <p className="text-xs text-slate-400 mt-0.5">Update the details for this item (stock is managed via transactions)</p>
                        </div>
                    </div>
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 shadow-lg">
                        <form onSubmit={submit} className="space-y-5">
                            ...
                            <Field label="SKU Code" error={errors.sku}>
                                <input type="text" className={inputCls} value={data.sku} onChange={e => setData('sku', e.target.value)} required />
                            </Field>
